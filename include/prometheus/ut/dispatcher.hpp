...
#include <prometheus/ut/operands.hpp>
#include <prometheus/ut/executor.hpp>

namespace prometheus::ut::dispatcher
{
	...
	{
		using type = L::dispatcher_type;
	};

	template<typename L, typename R>
	struct lazy_dispatcher_type<false, L, R>
	{
		using type = R::dispatcher_type;
	};

	template<bool C, typename L, typename R>
	using lazy_dispatcher_type_t = lazy_dispatcher_type<C, L, R>::type;
	...
	using lazy_expression_type_t = lazy_expression_type<C, T>::type;
	...
	public:
		...
		constexpr auto operator<<(const std::string_view message) noexcept -> ExpectResult&
		{
			if (not result_)
			{
				register_event(events::EventLog{.message = message});
			}
	...
	{
	public:
		// [[nodiscard]] constexpr static auto placeholder() noexcept -> DispatcherSuite
		// {
		// 	return {};
		// }
		//
		// template<std::invocable InvocableType>
		// constexpr auto operator=(InvocableType&& invocable) const noexcept(false) -> const DispatcherSuite& //
