...
#include <prometheus/functional/value_list.hpp>

#include <prometheus/math/cmath.hpp>

namespace prometheus::ut::operands
{
	class Operand
	{
	public:
	...
	template<typename T>
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandValue final : public Operand
	{
	public:
		using value_type = T;
	...
	template<typename T>
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandValueRef final : public Operand
	{
	public:
		using value_type = T;
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandLiteral : public Operand {};

	template<char Value>
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandLiteralCharacter final : public OperandLiteral
	{
	public:
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandLiteralIntegral final : public OperandLiteral
	{
	public:
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandLiteralFloatingPoint final : public OperandLiteral
	{
	public:
		using value_type = std::remove_cvref_t<decltype(Value)>;
		...
		constexpr static value_type epsilon = [](std::size_t n) noexcept -> value_type
		{
			...
			while (n--)
			{
				epsilon /= static_cast<value_type>(10);
			}
			return epsilon;
		}(DenominatorSize);

		[[nodiscard]] constexpr auto operator-() const noexcept -> OperandLiteralFloatingPoint<-value, DenominatorSize>
		{
			return {};
		}
	...
	template<char... Cs>
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandLiteralAuto final : public OperandLiteral
	{
	public:
		...
		template<typename T>
		using rebind = rep<std::remove_cvref_t<T>>::type;
	};
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandIdentityBoolean final : public Operand
	{
	public:
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandIdentityString final : public Operand
	{
	public:
	...
	public:
		...
		[[nodiscard]] friend constexpr auto operator==(const OperandIdentityString& identity, const StringType& string) noexcept -> bool //
		{
			...
			{
				PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE("In one of your `\"xxx\"_b == xxx` expressions, xxx does not support `operator==(std::string_view)`.");
			}
		}
		...
		[[nodiscard]] friend constexpr auto operator==(const StringType& string, const OperandIdentityString& identity) noexcept -> bool //
		{
			...
			{
				PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE("In one of your `xxx == \"xxx\"_b` expressions, xxx does not support `operator==(std::string_view)`.");
			}
		}
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandExpression final : public Operand
	{
	public:
		...
		[[nodiscard]] constexpr auto do_check() const noexcept -> bool
		{
			...
			) noexcept -> bool
			{
				if constexpr (category == ExpressionCategory::EQUAL)
				{
					...
					{
						PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE("Not comparable!");
					}
				}
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandThrow final : public Operand
	{
	public:
	...
	public:
		...
		[[nodiscard]] constexpr auto caught() const noexcept -> bool
		{
			return caught_;
		}
	...
	class PROMETHEUS_COMPILER_EMPTY_BASE OperandNoThrow final : public Operand
	{
	public:
