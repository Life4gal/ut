...
#include <prometheus/ut/events.hpp>
...
namespace prometheus::ut::executor
{
	/**
	 * Executor:
	...
	public:
		...
		~Executor() noexcept
		{
			if (not config_.dry_run)
			{
				...
				// create thread
				PROMETHEUS_PLATFORM_ASSUME(worker_pool_.empty());
...
#if defined(PROMETHEUS_PLATFORM_WINDOWS)
...
#define SYSTEM_CLEAR_CONSOLE std::system("clear"); // NOLINT(concurrency-mt-unsafe)
				...
				std::ranges::for_each(
					...
					{
						PROMETHEUS_PLATFORM_ASSUME(thread.joinable());
						thread.join();
					}
				);
		...
		[[nodiscard]] auto get_config() noexcept -> config_type&
		{
			return config_;
		}
	...
	private:
		...
		{
			PROMETHEUS_PLATFORM_ASSUME(suite_ != nullptr);
			return *suite_;
		}
		...
		{
			PROMETHEUS_PLATFORM_ASSUME(not suite().results.empty());

			if constexpr (Type == IdentType::ASSERTION)
			{
				PROMETHEUS_PLATFORM_ASSUME(current_test_result_ != nullptr);
			}
			else
			{
				// top level
		...
		auto run(const events::EventSuite& suite, suite_result_type& suite_result) noexcept -> void
		{
			...
			// ==================================
			// end/error
			PROMETHEUS_PLATFORM_ASSUME(current_test_result_ == nullptr or off_working());
		...
		{
			PROMETHEUS_PLATFORM_ASSUME(current_test_result_ != nullptr);
			PROMETHEUS_PLATFORM_ASSUME(current_test_result_->name == test_end.name);
		...
		{
			PROMETHEUS_PLATFORM_ASSUME(
				current_test_result_ != nullptr,
				"Anonymous test cases are not currently supported. You must place all test-expression under test case.(\"my_test\"_test = [] { expect(...); };)"
			);
		...
		auto on(const events::EventAssertionFail<Expression>& assertion_fail) noexcept(false) -> void
		{
			...
			PROMETHEUS_PLATFORM_BREAKPOINT_IF(executor().config_check_break_point<config_type::BreakPointLevel::FAILURE>(), "EventAssertionFail");
		...
		{
			PROMETHEUS_PLATFORM_BREAKPOINT_IF(executor().config_check_break_point<config_type::BreakPointLevel::FATAL>(), "EventAssertionFatal");
		...
		auto on(const events::EventLog& log, internal_tag) noexcept -> void
		{
			if (log.message != "\n")
			[[likely]]
			{
	...
	public:
		...
		auto on(const events::EventLog& log) noexcept -> void
		{
	...
	{
		PROMETHEUS_PLATFORM_ASSUME(suites_.size() == suite_results_.size());
		     ...
		     const auto index_in_bucket: std::views::iota(static_cast<std::size_t>(0), bucket_count))
		{
			...
			PROMETHEUS_PLATFORM_ASSUME(index < suites_.size());
